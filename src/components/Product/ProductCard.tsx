...
const ProductCard: React.FC<ProductProps> = ({ data, type }) => {
  ...
  const handleAddToCart = () => {
    if (
      !cartState.cartArray.find((item) => item.ProductID === data.ProductID)
    ) {
      addToCart({ ...data });
      updateCart(
        data.ProductID,
  ...
  const handleAddToCompare = () => {
    ...
    if (compareState.compareArray.length < 3) {
      if (
        ...
        removeFromCompare(data.ProductID);
      } else {
        // else, add to wishlist and set state to true
        addToCompare(data);
      }
  ...
  let percentSale = Math.floor(100 - (data.ProdPriceWithTax / data.ProdPrice) * 100);
  let percentSold = Math.floor((15 / data.Stocks) * 100);
    ...
    <>
      {type === "grid" ? (
              ...
              <div className="product-sold sm:pb-4 pb-2">
                ...
                <div className="flex items-center justify-between gap-3 gap-y-1 flex-wrap mt-2">
                  <div className="text-button-uppercase">
                    ...
                    <span className="max-sm:text-xs">{data.Sale}</span>
                  </div>
                  <div className="text-button-uppercase">
                    <span className="text-secondary2 max-sm:text-xs">
                      Available:{" "}
                    </span>
                    <span className="max-sm:text-xs">{data.Stocks - 2}</span>
                  </div>
                </div>
              </div>
                ...
                {percentSale > 0 && (
                  <>
                    <div className="product-origin-price caption1 text-gray-700">
                      <del> ${data.ProdPriceWithTax}.00</del>
                    </div>
        ...
        <>
          {/* {type === "list" ? (
            <>
              <div className="product-item list-type bg-red">
